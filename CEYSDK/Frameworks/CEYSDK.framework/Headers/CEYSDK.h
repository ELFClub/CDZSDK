...
#import "CEYUserModel.h"

@interface CEYSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(CEYCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(CEYCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，CEYSDKDefines中有声明
 */
- (void)setLanguage:(CEYLanguageType)languageType;
 ...
 *
 */
- (void)login:(CEYCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(CEYCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(CEYCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(CEYCallbackBlock)Block;
 ...
 */
- (void)userInfo:(CEYCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(CEYCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(CEYCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface CEYSDK(CEYIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(CEYCallbackBlock)block ;
