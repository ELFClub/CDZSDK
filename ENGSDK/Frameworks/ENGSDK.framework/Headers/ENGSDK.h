...
#import "ENGSDKDefines.h"
#import "ENGUserModel.h"

@interface ENGSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(ENGCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(ENGCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，ENGSDKDefines中有声明
 */
- (void)setLanguage:(ENGLanguageType)languageType;
 ...
 *
 */
- (void)login:(ENGCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(ENGCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(ENGCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(ENGCallbackBlock)Block;
 ...
 */
- (void)userInfo:(ENGCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(ENGCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(ENGCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface ENGSDK(ENGIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(ENGCallbackBlock)block ;
