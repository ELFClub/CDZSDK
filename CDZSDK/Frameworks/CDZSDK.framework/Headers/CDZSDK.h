...
#import "CDZUserModel.h"

@interface CDZSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(CDZCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(CDZCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，CDZSDKDefines中有声明
 */
- (void)setLanguage:(CDZLanguageType)languageType;
 ...
 *
 */
- (void)login:(CDZCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(CDZCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(CDZCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(CDZCallbackBlock)Block;
 ...
 */
- (void)userInfo:(CDZCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(CDZCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(CDZCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface CDZSDK(CDZIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(CDZCallbackBlock)block ;
