...
#import "CFWSDKDefines.h"
#import "CFWUserModel.h"

@interface CFWSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(CFWCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(CFWCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，CFWSDKDefines中有声明
 */
- (void)setLanguage:(CFWLanguageType)languageType;
 ...
 *
 */
- (void)login:(CFWCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(CFWCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(CFWCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(CFWCallbackBlock)Block;
 ...
 */
- (void)userInfo:(CFWCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(CFWCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(CFWCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface CFWSDK(CFWIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(CFWCallbackBlock)block ;
